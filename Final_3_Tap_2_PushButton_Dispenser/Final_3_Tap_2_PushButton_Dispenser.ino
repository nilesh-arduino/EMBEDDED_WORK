...
//########################################################################################################################
//#                                <<   SMART THREE TAP TWO PUSH BUTTON BASED DISPENSER  >>                              #
//# Developer:Nilesh Vyas                                                                                                #
//#                                                                                                                      #
//#                                                                                                                      #
//# << Dispenser Process >>                                                                                              #
//# 3 Tap 2 Push Button based Dispenser                                                                                  #
//# 1st Push Button On first Tap and dispensed 20 L Water                                                                #
//# 2nd Push Button On second and third tap and dispensed 1 Liter Water                                                  #
//########################################################################################################################
...
#define SV1_PIN                         39//41
#define SV2_PIN                         41//39
#define SV3_PIN                         37//33
#define FLOWSENSOR_1_PIN                23
#define FLOWSENSOR_2_PIN                25
#define FLOWSENSOR_3_PIN                27
#define BOOSTER_PUMP_PIN                33
#define OZONATOR_PIN                    35//37
#define RST_PIN                         49
#define SS_PIN                          53
...
enum 
{
  ...
  TSDD_U,     //1.timstamp dispensing data higher 2 bytes
  TSDD_L,     //2.time stamp dispensing data lower 2 bytes
  TVD_U,      //3.total volume dispensed upper 2 bytes
  TVD_L,      //4.total volume dispensed lower 2 bytes
  TRCH_U,     //5.total recharge done upper 2 bytes
  TRCH_L,     //6.total recharge done lower 2 bytes
  TCR_U,      //7.total collection RFID upper 2 bytes
  TCR_L,      //8.total collection RFID lower 2 bytes
  TCC_U,      //9.total collection coin upper 2 bytes
  TCC_L,      //10.total collection coin lower 2 bytes
  TCS_U,      //11.total collection sale upper 2 bytes
  TCS_L,      //12.total collection sale lower 2 bytes
  TCV1_U,     //13.total count vol1 upper 2 bytes
  TCV1_L,     //14.total count vol1 lower 2 bytes
  TCV2_U,     //15.total count vol2 upper two bytes
  TCV2_L,     //16.total count vol2 lower two bytes
  TCV3_U,     //17.total count vol3 upper two bytes
  TCV3_L,     //18.total count vol3 lower two bytes
  TCV4_U,     //19.total count vol4 upper two bytes
  TCV4_L,     //20.total count vol4 lower two bytes
  TCV5_U,     //21.total count vol5 upper two bytes
  TCV5_L,     //22.total count vol5 lower two bytes
  TCCV1_U,    //23.total coin count value1 upper two bytes
  TCCV1_L,    //24.total coin count value1 lower two bytes
  TCCV2_U,    //25.total coin count value2 upper two bytes
  TCCV2_L,    //26.total coin count value2 lower two bytes
  PHOW,       //27.pH of water
  TLV,        //28.Tank level
  TOW,        //29.temperature of water
  LAT_U,      //30.lattitude
  LAT_L,      //31.
  LONG_U,     //32.longitude
  LONG_L,     //33.
  ...
  FLAG2,      //35.transaction log status  
  TSTL_U,     //36.timestamp higher 2 bytes
  TSTL_L,     //37.timestamp lower 2 bytes
  TN,         //38.tap no
  TOR,        //39.type of request
  TID_U,      //40.transaction Id upper 2 bytes
  TID_L,      //41.transaction Id lower 2 bytes
  QT,         //42.quantity requested
  AM,         //43.amount for the quantity
  DT,         //44.dispense time in seconds
  CB_U,       //45.card balance upper 2 bytes
  CB_L,       //46.card balance lower 2 bytes
  TR_U,       //47.total rechage upper 2 bytes
  TR_L,       //48.total recharge lower 2 bytes
  ED_U,       //49.expiry date upper 2 bytes
  ED_L,       //50.expiry date upper two bytes  

  //-------------------registers for operator login/logout-----------------------//
  FLAG3,      //51.operator punch status
  OPID_U,     //52.operatoe punch id higher 2 bytes
  OPID_L,     //53.operator punch id lower 2 bytes
  PUNT_U,     //54.punch time higher 2 bytes
  PUNT_L,     //55.punch time lower 2 bytes

  //-----------------registers for configure parameters--------------------------//
  SFLG,       //56.registers for configuration parameters
  RFLG,       //57.reset flag to set parameters if changed remotrly
  TV1,        //58.tariff volume1
  TV2,        //59.tariff volume2
  TV3,        //60.tariff volume3
  TV4,        //61.tariff volume4
  TV5,        //62.tariff volume5
  ACV1,       //63.allowed coin value1
  ACV2,       //64.allowed coin value2
  WV1,        //65.water volume1
  WV2,        //66.water volume2
  WV3,        //67.water volume3
  WV4,        //68.water volume4
  WV5,        //69.water volume5
  VCV1,       //70.volume coin value1
  VCV2,       //71.volume coin value2
  PV1,        //72.pulse coin value1
  PV2,        //73.pulse coin value2
  PV3,        //74.pulse coin value3
  PV4,        //75.pulse coin value4
  PV5,        //76.pulse coin value5
  PCV1,       //77.pulse coin value1
  PCV2,       //78.pulse coin value2
  CPV1,       //79.coin pulse value1
  CPV2,       //80.coin pulse value2
  SNUB_U,     //81.serial no upper bound higher 2 bytes
  SNUB_L,     //82.serial no upper bound lower 2 bytes
  SNLB_U,     //83.serial no lower bound higher 2 bytes
  SNLB_L,     //84.serial no lower bound lower 2 bytes
  OSN_U,      //85.operator serial no higher 2 bytes
  OSN_L,      //86.operator serial no lower 2 bytes
  SSN_U,      //87.supervisor serial no higher 2 bytes
  SSN_L,      //88.supervisor serial no lower 2 bytes
  ASN_U,      //89.admin serial no higher 2 bytes
  ASN_L,      //90.admin serial no lower 2 bytes
  MSC,        //91.machine state control

//-----------------registers for supervisor set/reset------------------------------//
  FLAG4,      //92.flag for supervisor set/reset status
  TSTC_U,     //93.time stamp higher 2 bytes
  TSTC_L,     //94.time stamp lower 2 bytes
  TCID_U,     //95.trip collection id higher 2 bytes
  TCID_L,     //96.trip collection id lower 2 bytes
  TTRC,       //97.trip total reset count
  TTVD,       //98.trip total volume dispensed
  TTSC,       //99.trip total smart card collection
  TTCC,       //100.trip total coin collection
  TTCR,       //101.trip total card recharge
  TTCS,       //102.trip total collection sale
  TAAC,       //103.trip actual amount collection
...
/**************************************************************************************************/
  ...
  class settingParameter 
   {  
      ...
      bool machineStateControl,bypassRTC;
...
void settingParameter::setData()
 {    
       ...
       longitude            =   LONGITUDE;   
       alltitude            =   ALTITUDE; 
       bypassRTC            =   BYPASS_RTC;
 }
           
/**************************************************************** END OF SETTING PARAMETER ******************************************************/
    ...
    {      
      ...
      if (IsDispensing == yes && dispense == yes)
      {
          ...
          if (currentMillis- dispenseMillis >= dispenseTimeout)//The value of dispenseTimeout can be configured.
          {
              ...
              TOTAL_VOLUME_DISPENSED += flowVolume;       //in ml
              flowVolume = 0;
              lcd1.clear();
...
WATER_ATM Tap3(SV3_PIN, FLOWSENSOR_3_PIN, OZONATOR_PIN, BOOSTER_PUMP_PIN);

/****************************** END OF OBJECT DECLERATION **********************/


/******************************** GLOBAL VARIABLE FOR DISPENSING ***********************************************************************************/
...
bool lastState_IsDispensing_tap1,lastState_IsDispensing_tap2,lastState_IsDispensing_tap3;

/******************************** END OF GLOBAL VARIABLE FOR DISPENSING ******************************************************************************/
...
SIGNAL(TIMER0_COMPA_vect) 
{
      ...
      if(currentMillis-samplingMillis>=10)
      {
            samplingMillis=currentMillis; 
            
            //call tapUpdate here
            if((Tap1.IsDispensing==HIGH)&&(Tap2.IsDispensing==HIGH)&&(Tap3.IsDispensing==HIGH))
            {
               lastState_IsDispensing_tap1=Tap1.IsDispensing;
               lastState_IsDispensing_tap2=Tap2.IsDispensing;
               lastState_IsDispensing_tap3=Tap3.IsDispensing;
               ...
               pushbuttonDetect();
                ...
                if(Button4==LOW)
                {
                      Tap2.lastBreakState = Tap3.lastBreakState=Button4;
                      Tap2.BREAK_DISPENSE = Tap3.BREAK_DISPENSE=LOW;
                }
                else if(Button4 == HIGH)
                {
                      Tap2.BREAK_DISPENSE = Tap3.BREAK_DISPENSE= Button4;
                      //Tap2.stopBooster();
                }
               Tap1.Update();
               Tap2.Update();
            ...
            else if(Tap1.IsDispensing==HIGH && Tap2.IsDispensing==HIGH)
            {
                lastState_IsDispensing_tap2=Tap2.IsDispensing;
                lastState_IsDispensing_tap1=Tap2.IsDispensing;
             ...
             else if(Tap1.IsDispensing==HIGH && Tap3.IsDispensing==HIGH)
            {
                lastState_IsDispensing_tap3=Tap3.IsDispensing;
                lastState_IsDispensing_tap1=Tap1.IsDispensing;
             ...
             else if(Tap2.IsDispensing==HIGH && Tap3.IsDispensing==HIGH)
            {
                lastState_IsDispensing_tap2=Tap2.IsDispensing;
                lastState_IsDispensing_tap3=Tap3.IsDispensing;
            ...
            else if(Tap2.IsDispensing==HIGH)
            {
             ...
             else if(Tap3.IsDispensing==HIGH)
            {
                lastState_IsDispensing_tap3=Tap3.IsDispensing;
                Tap3.Update();
                 pushbuttonDetect();
...
void loop()
{
      ...
      if(currentMillis-dispenseloopMillis>=DISPENSE_LOOP_RATE)
      {
            ...
            if(Tap1.scan==HIGH || Tap2.scan==HIGH && Tap3.scan==HIGH)
            {   
                if(ERROR_CODE>0)
                {
                    ERROR_CODE=0;
                ...
                if(Tap1.IsDispensing!=HIGH || Tap2.IsDispensing!= HIGH && Tap3.IsDispensing!= HIGH)
                {
                    ...
                    {
                        Serial.println("TAP2");
                        Tap2.lastBreakState = Tap3.lastBreakState=Tap2.BREAK_DISPENSE;
                        Tap2.BREAK_DISPENSE =  Tap3.BREAK_DISPENSE= LOW;
                        Tap2.startDispensing(2000,get_waterVolume(2),getpulseRequired(2));
                        Tap3.startDispensing(2000,get_waterVolume(2),getpulseRequired(2));
                        Serial.println("Tap3 start");
    ...
    if(Tap1.dispense==HIGH || Tap2.dispense==HIGH || Tap3.dispense==HIGH)
    { 
        if(currentMillis-dispensedisplayMillis>=500)
        {
            ...
            if(Tap1.dispense==HIGH && Tap2.dispense==HIGH && Tap3.dispense==HIGH)
            {
                  ...
                  lcd1.setCursor(9,0);
                  lcd1.print("T3: ");
                  lcd1.print(Tap3.flowVolume);
            }
             else if(Tap1.dispense==HIGH && Tap2.dispense==HIGH)
            {
                  lcd1.setCursor(0,0);
                  lcd1.print("T1: ");
                  lcd1.print(Tap1.flowVolume);
            ...
            else if(Tap1.dispense==HIGH && Tap3.dispense==HIGH)
            {
                  lcd1.setCursor(0,0);
                  lcd1.print("T1: ");
                  lcd1.print(Tap1.flowVolume);
            ...
            else if(Tap2.dispense==HIGH && Tap3.dispense==HIGH)
            {
                  lcd1.setCursor(0,0);
                  lcd1.print("Tap2: ");
                  lcd1.print(Tap2.flowVolume);
 ...
 /*   if((Tap1.IsDispensing!=HIGH) && (Tap2.IsDispensing!=HIGH))
    ...
    if((Tap1.IsDispensing==LOW && lastState_IsDispensing_tap1==HIGH) || (Tap2.IsDispensing==LOW && lastState_IsDispensing_tap2==HIGH) || (Tap3.IsDispensing==LOW && lastState_IsDispensing_tap3==HIGH)  )
    {
          ...
          else if(Tap2.IsDispensing==LOW && lastState_IsDispensing_tap2==HIGH && Tap3.IsDispensing==LOW && lastState_IsDispensing_tap3==HIGH)
          {
                lastState_IsDispensing_tap2=Tap2.IsDispensing;
                lastState_IsDispensing_tap3=Tap3.IsDispensing;
                if(Tap2.dispensingComplete==HIGH && Tap3.dispensingComplete==HIGH)
                {
                    TOTAL_VOLUME_DISPENSED += Tap2.Dispensed_Volume;
                    TOTAL_VOLUME_DISPENSED += Tap3. Dispensed_Volume;
                    lcd1.clear();
                    lcd1.setCursor(2,0);
                    lcd1.print("Dispensing");
                    lcd1.setCursor(3,1);
                    lcd1.print("Complete");
                    Serial.print("TOTAL VOLUME DISPENSED");
                    Serial.println(TOTAL_VOLUME_DISPENSED);
          ...
          else if(Tap2.IsDispensing==LOW && lastState_IsDispensing_tap2==HIGH)
          {
                ...
                {
                    TOTAL_VOLUME_DISPENSED += Tap2.Dispensed_Volume;
                    //TOTAL_VOLUME_DISPENSED += (Tap3. Dispensed_Volume)/1000;
                    lcd1.clear();
                    lcd1.setCursor(2,0);
                    lcd1.print("Dispensing");
                    lcd1.setCursor(3,1);
           ...
           else if(Tap3.IsDispensing==LOW && lastState_IsDispensing_tap3==HIGH)
           {
                lastState_IsDispensing_tap3=Tap3.IsDispensing;
                if(Tap3.dispensingComplete==HIGH)
                {
...
/****************************************************** END OF DISPLAY TIME FUNCTION ******************************/

/****************************************************** PUSH BUTTON DETECT  FUNCTION ******************************/
...
/**************************************************END OF PUSH BUTTON DETECT  FUNCTION ******************************/


/********************************************* WATER VOLUME SELECTION IN LITER FUNCTION ******************************/ 
...
/********************************************* END OF  WATER VOLUME SELECTION IN LITER FUNCTION ****************************/
...
/*****************************************************END OF DISPENSE DISPLAY FUNCTION **********************************************/
...
/***************************************************** END OF EEPROM READ AND WRITE FUNCTION ********************************************/   
...
/***************************************************** FUNCTION FOR CONTROLLER SETTING **************************************************/
...
/*****************************************************ENF OF  FUNCTION FOR CONTROLLER SETTING ***********************************************/


/***************************************************** CALCULATION OF  PULSE REQUIRED  ******************************************************/                           
...
/***************************************************** END OF CALCULATION OF PULSE REQUIRED  ******************************************************/ 
...
/***************************************************** UPDATE DISPENSING DATA ON MODBUS   ******************************************************/ 
...
/***************************************************** END OF UPDATE DISPENSING DATA ON MODBUS ******************************************************/  

/***************************************************** SET RTC TIME AND TIMESTAMP FUNCTION **********************************************************/                               
...
/***************************************************** END OF SET RTC TIME AND TIMESTAMP FUNCTION ********************************************************/
